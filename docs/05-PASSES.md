...
1. **Promise** — the opening makes the contract (en-medias-res or Godfather-style; a shard of the destination or the storm). No valid story opens without one. **The antagonist IS a promise (user law, 2026-07-15):** fully showing a crazy/ungovernable antagonist is itself the opening contract — the audience supplies "something must answer this" on their own; no solution-teasing needed in the open (Brehon walkthrough: the 40,000-word Wikipedia war told as a story IS the promise).
...
4. **Doorways** — Act 1 and Act 2 breaks present and in order (conception-level: the spine gate, `studio/src/DramaGate.res`; a dedicated doorway runner is still owed).
...
**THE STACK RUNS AS ONE COMMAND (2026-07-16):** `CLAUDE_STUDIO_BUDGET=8 node studio/src/Battery.res.mjs <scene.txt>` — mechanical floor (craft devices, echo, show/ornament on action lines), then the judges in stage order (Clarity → Drama → SceneCraft core → HumanReaction). Substance BLOCKS (exit 1); `--finishing` runs the One Law as triage and never blocks. Writes `<scene>.battery.txt`. A pass means "worth the author's read" — the human is the stop. Fixtures for every gate live in `Gauntlet.res`; every new user catch gets fossilized there the same day.
...
10. **Humanizer (the slop pass)** — the kill-list: stapled-fragment narration, rule-of-three stacks, "not X but Y" parallelism, mic-drop closers, em-dash chains, purple verbs, poignancy-reach endings — scenes end FLAT; plus the deeper tell: clipped/punchy/buttoned rhythm itself (userSettings:humanizer + docs/03 §2).
    ...
    **ENFORCEMENT IS MECHANICAL (2026-06-11, after the third miss):** the prose gates run as CODE
    (`studio/src/Gate.res` craftlint + `Craft.res` device regexes + `TellsGate.res` for expository
    deliverables) on every changed chapter BEFORE any deliverable is staged — the
...
## Prose deliverables (treatments, summaries, pitch copy, editor reads, docs)
The expository register has its own tells — the AI hand in a suit. **The named-tells registry
lives in `studio/src/Tells.res`** (single source: the 16 named tells — hedge stack, preamble
promise, transition summary, enthusiasm injection, corporate verb, passive distancer, closer
summary, delve/dive, realm-of, the landing, pivot closer, em-dash itemizer, exact-match claim, <!-- tells:ok -->
reality reframe, gerund pedestal, nested qualification — merged with the house bans and the
**Meeting Leak** lint from the transcript-artifact law: "as requested / per your feedback / <!-- tells:ok -->
we decided / no longer uses" = the conversation leaking into the artifact). Quoted examples <!-- tells:ok -->
in a law doc carry the `tells:ok` pragma (use/mention — the gate exempts the line). **Run
`node studio/src/TellsGate.res.mjs <file.md>` on EVERY prose deliverable before it ships**;
Block-tier hits stop the send. Dialogue and scenes are exempt by construction — people hedge;
that's human speech. The judge tier (soft landings, restatement closers, asserted fit) prints
with each run and belongs to the finishing read.
