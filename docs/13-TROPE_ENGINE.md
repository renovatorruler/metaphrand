...
| TropeTagger.res | tag a story; every tag needs a verified on-page quote | YES (warm Session, budget-capped) |
...
CLAUDE_STUDIO_BUDGET=<cap> node src/TropeTagger.res.mjs run-all   # REAL spend
...
CLAUDE_STUDIO_BUDGET=<cap> node src/TropeGate.res.mjs check <brief_id> <draft>  # REAL spend
```
