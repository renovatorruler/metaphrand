...
def build() -> Story:
    ...
    # -- the MIRROR is the Midpoint (the rebirth rite) ----------------

    # -- AFTER: the man who found a self ------------------------------
    s.instantiate(after.id, "Nora corners him outside the gallery with her brother's unmailed letters; she's after the truth, and she'll take nothing performed, which no one has ever asked of him before",
                  kind="beat", id="b-nora", attributes={"function": "B Story", "character": "NORA", "dialogue": "yes"})
    s.instantiate(after.id, "Deeper in: the work, the table they keep for him, Marsh walking him through the loft with a hand on his shoulder, the badge's calls going to voicemail, the cover no longer coming off at the door",
                  kind="beat", id="b-fun", attributes={"function": "Fun and Games"})
    # flesh: the world breathing
    s.instantiate(after.id, "The machine runs in plain sight: members sign their savings over as patronage, the gallery launders it, and the work leaves the building under Marsh's name and comes back as money",
                  kind="beat", id="b-money")
    s.instantiate(after.id, "Priya, twenty and gifted, takes the chair at the center of the Seeing, the chair Eli sat in four years back, and lets them name the great one she is going to be",
                  kind="beat", id="b-priya")
    ...
    s.instantiate(after.id, "Cowan keeps the second set of books and handles the members who crack; he clocks the cop in Danny before anyone else and starts watching the doors",
                  kind="beat", id="b-cowan")
    s.instantiate(after.id, "August, a dentist who sold his practice to come inside, breaks down in the circle as they tell him he was never ordinary, and writes another check",
                  kind="beat", id="b-august")
    s.instantiate(after.id, "Reyes tapes the wire to his chest: walk in, give her Marsh, burn it down -- and to keep a badge that was never any realer, he has to torch the only self that was ever his",
                  kind="beat", id="b-wire", attributes={"doorway": 2})
    s.instantiate(after.id, "He stands at the rite with the wire live under his shirt and cannot say which of the two men it is wired to; the gift pulling him under is the exact gift that put Eli in the river",
    ...
    s.instantiate(after.id, "Danny drives past the house that used to be his, his ex-wife's car in the drive of a life that went on without him, and doesn't slow down",
                  kind="beat", id="b-maria")
    s.instantiate(after.id, "The one question that always stopped him cold, and standing in the rite with the wire live he opens his mouth and answers it out loud for the first time in his life",
                  kind="beat", id="b-what", attributes={"function": "Break into Three"})
    s.instantiate(after.id, "Not for the badge and not for Marsh: Danny pulls Priya out of the loft before the machine closes on her and burns the whole con down with his own name on the record",
