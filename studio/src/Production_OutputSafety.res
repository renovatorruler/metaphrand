/* Generic filesystem guards for the production control plane.

   Manifest-controlled outputs are allowed only beneath their declared base
   directory. Both the lexical path and its nearest existing real ancestor are
   checked so an existing symlink cannot redirect a write outside that tree.
   Collision comparisons use the same projected real path. */
...
/* Treat case-only aliases as collisions even on a case-sensitive development
   volume so a diagnostic set cannot become destructive when moved between
   filesystems. */
...
let manifestOutputPath = (~baseDir: string, ~relativePath: string, ~label: string): string => {
  ...
  if isAbsolute(relativePath) {
    raise(OutputSafetyError(label ++ " must be relative to the base directory: " ++ relativePath))
  }
  ...
  if lexicalRelative == "" || escapes(lexicalRelative) {
    raise(OutputSafetyError(label ++ " escapes or names the base directory: " ++ relativePath))
  }
  ...
  if realRelative == "" || escapes(realRelative) {
    raise(
      OutputSafetyError(
        label ++ " escapes the base directory through an existing path: " ++ relativePath,
      ),
    )
  }
  candidate
  ...
  outputs->Belt.Array.forEachWithIndex((i, output) => {
    outputs->Belt.Array.forEachWithIndex((j, other) =>
      if j > i && pathsCollide(output.path, other.path) {
        raise(
          OutputSafetyError(
            output.label ++ " collides with " ++ other.label ++ ": " ++ canonicalIdentity(output.path),
          ),
        )
      }
    )
