// KukuEp10_Sets.res — the «द से दीया» courtyard: its plate, and the shots on it.
//
// EP10 has ONE set, so it has one master plate, and every shot in the episode is
// generated against that plate. This is the lesson the ग-से-गाय attempt paid for twice: when a
...
//
//   node src/KukuEp10_Sets.res.mjs plate     # the courtyard, dusk
//   node src/KukuEp10_Sets.res.mjs gate      # prompt check, no spend
...
/* EP10 USES ITS OWN STYLE KEY. The shared default is a crop of the rejected ग-से-गाय lane, and
   it leaked that lane — red markers and all — into a courtyard shot. This one is
   a landmark-free patch of EP10's own wall and flagstone: paper material with no
   place in it. */
...
/* The paper MATERIAL, taken from a Higgsfield 3D-Papercraft render of our own
   ...
   it donates material and nothing else — the ग-से-गाय style key taught us that a key
...
let doPlate = () => {
  ...
  ignore(
    Kuku_Engine.plate(~episode="EP10", ~id="ep10_courtyard_plate", ~prompt=platePrompt, ~refs, ~dst=platePath, ()),
  )
}
...
let doClip02 = () =>
  ignore(
    Kuku_Engine.clip(
      ...
      ~dst=root ++ "clips/EP10_e01_dadi_lights.mp4",
      (),
    ),
  )
