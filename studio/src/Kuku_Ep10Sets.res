...
let vantageProse = (s, v) =>
  ...
  | (Lane, GroundLevel) => "we look from low on the flat stretch at the BOTTOM of the lane, at head height, back along the last level metres: the closed stone wall stands across the far end, the slope climbing away behind it, the lane's kerbs running up to meet it"
...
let lanePlateAt = y =>
  if y < 30.0 {
    ...
    /* the lower lane's plate is the GroundLevel vantage, generated by the set
       bible like any other. It used to be a hand-copied file with no receipt,
       which the engine rightly refused to edit. */
    platePath(Lane, "ground_level")
  }
...
let lanePosition = (at, cartAt) => {
  ...
  let base =
    ...
    Belt.Float.toString(toWall) ++ (toWall < 10.0
      ? " metres further on — close ahead now, filling the view."
      : toWall < 28.0
      ? " metres further on — clearly ahead down the slope."
      : " metres further on — far down the lane, a distant line.")
  switch cartAt {
  | None => base
  | Some(c) =>
    base ++
