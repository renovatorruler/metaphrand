/* KukuEp10Gaay_Package.res — emit a self-contained PROMPT PACKAGE for a shot.
   ...
   Run from studio/:
     node src/KukuEp10Gaay_Package.res.mjs h22_castor_calm       # one still shot
     node src/KukuEp10Gaay_Package.res.mjs s2c_castor_talks      # one clip
     node src/KukuEp10Gaay_Package.res.mjs --all                 # every shot */
...
let packShot = (e: KukuEp10Gaay_Shots.entry) => {
  let dir = root ++ e.id
...
let packClip = (c: KukuEp10Gaay_SceneClips.clip) => {
  let dir = root ++ c.tag
  mkdirSync(dir, {"recursive": true})
...
let dirs = []
Js.Array2.forEach(KukuEp10Gaay_Shots.shots, e =>
  if wantAll || Js.Array2.includes(names, e.id) {
    Js.Array2.push(dirs, packShot(e))->ignore
  }
)
Js.Array2.forEach(KukuEp10Gaay_SceneClips.all, c =>
