...
try {
  ...
  let result = Kuku_Ep9B05Local.build(~root)
  Js.Console.log("B05 local motion: " ++ result.output)
  Js.Console.log("sha256: " ++ result.outputSha256)
  Js.Console.log("contact: " ++ result.contact)
  Js.Console.log(result.probe)
} catch {
| Kuku_Ep9B05Local.B05LocalError(message)
| Cinema_Backends.BackendError(message) => {
    Js.Console.error("B05 LOCAL MOTION FAILED: " ++ message)
    exitProcess(1)
  }
}
