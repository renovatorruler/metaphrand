...
@get external sizeOf: stat => float = "size"
...
let contentType = (name: string): string =>
  if Js.String2.endsWith(name, ".mp3") {
    "audio/mpeg"
  } else if Js.String2.endsWith(name, ".mp4") {
    "video/mp4"
  } else if Js.String2.endsWith(name, ".m4a") {
    "audio/mp4"
  } else if Js.String2.endsWith(name, ".png") {
    "image/png"
  } else if Js.String2.endsWith(name, ".jpg") || Js.String2.endsWith(name, ".jpeg") {
    "image/jpeg"
  ...
  let rows =
    ...
    ->Belt.Array.map(f => {
      let mb = Js.Float.toFixedWithPrecision(
        sizeOf(statSync(dir ++ "/" ++ f)) /. 1048576.0,
        ~digits=1,
      )
    ...
    ->Belt.Array.joinWith("\n", x => x)
  let title = Js.Dict.get(env, "TITLE")->Belt.Option.getWithDefault("dailies shelf")
