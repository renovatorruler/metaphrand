/* Frosya and Vasya — Episode 2 full-cast Russian table read, v4 pipeline.
   ...
   Run from studio/:
     DRY=1 node src/Drakosha_Ep2RussianTableReadV4.res.mjs
     PAID=1 node src/Drakosha_Ep2RussianTableReadV4.res.mjs */
...
let dir = "../stories/drakosha/audio/ep2_table_read_ru_v4"
let screenplay = "../stories/drakosha/2026-08-30_EP2_samyi-krutoi-nomer_SPEC_v3_numbered_bilingual.md"
let planPath = dir ++ "/ep2_table_read_ru_v4_plan.json"
let renderPath = dir ++ "/EP2_RUSSIAN_FULL_CAST_V4.manifest.json"
let outputPath = dir ++ "/EP2_RUSSIAN_FULL_CAST_V4.mp3"
...
let audioPipelineVersion = "drakosha-ep2-russian-table-read-v4"
...
let cast: array<castMember> = [
  {
    role: "NARRATOR",
    voice: "Denis - Native Russian Narrator",
    voiceId: "76rT6drChOHgGsscaHMJ",
    status: "native-Russian table-read narrator; replaces unapproved Anna Zub",
  },
  {
    role: "FROSYA",
...
let idFrom = (s: string): option<string> => {
  let i = Js.String2.indexOf(s, "E2V3-")
...
let sceneForHeading = (id: string): option<string> =>
  switch id {
  | "E2V3-001" => Some("title")
  | "E2V3-004" => Some("scene_01")
  | "E2V3-037" => Some("scene_02")
  | "E2V3-045" => Some("scene_03")
  | "E2V3-054" => Some("scene_04")
  | "E2V3-082" => Some("scene_05")
  | "E2V3-108" => Some("scene_06")
  | "E2V3-124" => Some("scene_07")
  | "E2V3-150" => Some("scene_08")
  | "E2V3-171" => Some("scene_09")
  | "E2V3-193" => Some("scene_10")
  | "E2V3-212" => Some("scene_11")
  | _ => None
  }
...
let stripMarkdown = (s: string): string =>
  ...
  ->Js.String2.replaceByRe(%re("/^#+ */g"), "")
  ->Js.String2.replaceByRe(%re("/^E2V3-[0-9]{3} *[·—] */g"), "")
  ->trim
...
let narrationFrom = (id: string, scene: string, markdown: string): (string, string) => {
  let plain =
    ...
    ->trim
  let fixed = switch id {
  | "E2V3-001" => "«Фрося и Вася». Серия вторая: «Самый крутой номер»."
  | _ => plain
  }
...
let parseScreenplay = (): array<segment> => {
  ...
  ->Belt.Array.forEach(raw => {
    ...
    let isNumbered = Js.String2.startsWith(raw, "**E2V3-")
...
let validate = (segments: array<segment>, chunks: array<chunk>): unit => {
  if Belt.Array.length(segments) != 217 {
    raise(TableRead(
      "expected 217 spoken spec blocks, got " ++ Belt.Int.toString(Belt.Array.length(segments)),
    ))
  }
  ...
  if Belt.Array.length(dialogue) != 114 || Belt.Array.length(narration) != 103 {
    raise(TableRead(
      "expected 114 dialogue and 103 narration blocks, got " ++
