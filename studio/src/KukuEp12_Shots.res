// KukuEp12_Shots.res — every shot of «द से दीया», staged against the one plate.
//
...
//
//   node src/KukuEp12_Shots.res.mjs gate      # every finding, no spend
//   node src/KukuEp12_Shots.res.mjs list
//   node src/KukuEp12_Shots.res.mjs still <id> | stills | clip <id>
...
let doStill = r => ignore(Kuku_Engine.still(~episode="EP12", ~id=r.id, ~spec=specOf(r), ~dst=stillPath(r), ()))
...
let clipPath = r => clipsDir ++ "EP12_" ++ r.id ++ ".mp4"
...
let doClip = r =>
  ...
  | Kuku_Engine.Current =>
    ignore(
      Kuku_Engine.clip(~episode="EP12", ~id=r.id, ~spec=clipSpecOf(r),
...
let () = {
  ...
  | _ => {
      ...
      if Js.Array2.length(bad) == 0 {
        Js.log("PROMPT GATE CLEAN — " ++ Belt.Int.toString(Js.Array2.length(shots)) ++ " EP12 shots obey the law")
      } else {
