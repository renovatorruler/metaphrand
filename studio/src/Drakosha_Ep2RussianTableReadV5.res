/* Frosya and Vasya — Episode 2 full-cast Russian table read, v5 pipeline.
   ...
   Run from studio/:
     DRY=1 node src/Drakosha_Ep2RussianTableReadV5.res.mjs
     PAID=1 node src/Drakosha_Ep2RussianTableReadV5.res.mjs */
...
let dir = "../stories/drakosha/audio/ep2_table_read_ru_v5"
let screenplay = "../stories/drakosha/2026-08-30_EP2_samyi-krutoi-nomer_SPEC_v4_numbered_bilingual.md"
let planPath = dir ++ "/ep2_table_read_ru_v5_plan.json"
let renderPath = dir ++ "/EP2_RUSSIAN_FULL_CAST_V5.manifest.json"
let outputPath = dir ++ "/EP2_RUSSIAN_FULL_CAST_V5.mp3"
let cacheDir = dir ++ "/cache"

/* Short takes keep reactions, jokes, and action turns playable. The rejected
   long-form pass let performances flatten across twenty-line requests. */
let maxChunkChars = 720
let maxChunkSegments = 7
let audioPipelineVersion = "drakosha-ep2-russian-table-read-v5-dot-trigger"
...
let cast: array<castMember> = [
  {
    ...
    status: "author-approved Russian narrator; English narration remains rejected",
  },
  {
    role: "FROSYA",
  ...
  },
  {
    ...
    status: "silent in this draft; reserved placeholder only",
  },
  {
    role: "MUSYA",
    voice: "Ekaterina - Professional Female Russian",
...
let idFrom = (s: string): option<string> => {
  let i = Js.String2.indexOf(s, "E2V4-")
  if i < 0 || Js.String2.length(s) < i + 8 {
    None
  } else {
    let base = Js.String2.slice(s, ~from=i, ~to_=i + 8)
    let suffix = Js.String2.length(s) > i + 8
      ? Js.String2.slice(s, ~from=i + 8, ~to_=i + 9)
      : ""
    Some(base ++ (Js.String2.includes("ABCDEFGHIJKLMNOPQRSTUVWXYZ", suffix) ? suffix : ""))
  }
}

let idNumber = (id: string): int =>
  Js.String2.slice(id, ~from=5, ~to_=8)->Belt.Int.fromString->Belt.Option.getWithDefault(-1)
...
let sceneForHeading = (id: string): option<string> =>
  switch id {
  | "E2V4-001" => Some("title")
  | "E2V4-004" => Some("scene_01")
  | "E2V4-037" => Some("scene_02")
  | "E2V4-045" => Some("scene_03")
  | "E2V4-054" => Some("scene_04")
  | "E2V4-082" => Some("scene_05")
  | "E2V4-108" => Some("scene_06")
  | "E2V4-124" => Some("scene_07")
  | "E2V4-150" => Some("scene_08")
  | "E2V4-171" => Some("scene_09")
  | "E2V4-193" => Some("scene_10")
  | "E2V4-212" => Some("scene_11")
  | _ => None
  }
...
let stripMarkdown = (s: string): string =>
  ...
  ->Js.String2.replaceByRe(%re("/^#+ */g"), "")
  ->Js.String2.replaceByRe(%re("/^E2V4-[0-9]{3}[A-Z]? *[·—] */g"), "")
  ->trim

/* The letters remain orthographically correct in spelling beats. Only the
   completed spoken word is respelled for ElevenLabs stress and vowel reduction.
   These substitutions never touch the screenplay or on-screen typography. */
let pronunciationForAudio = Drakosha_Pronunciation.audioInText

let isSpellingText = (s: string): bool =>
  Js.String2.includes(s, "В-О-Л") ||
  Js.String2.includes(s, "В-О-Л-К") ||
  Js.String2.includes(s, "СО-ВА") ||
  Js.String2.includes(s, "Л-А-В-К-А") ||
  Js.String2.includes(s, "О-Б-В-А-Л") ||
  Js.String2.includes(s, "М-О-С-Т") ||
  Js.String2.includes(s, "С-Т-В-О-Л")
...
let performanceTag = (direction: string): string => {
  ...
  if Js.String2.includes(d, "шёп") || Js.String2.includes(d, "тихо") {
    ...
    "[strained]"
            ...
            Js.String2.includes(d, "решившись") ||
            Js.String2.includes(d, "удерживая голос") ||
            Js.String2.includes(d, "вызывая") ||
    ...
    "[upset]"
  } else if Js.String2.includes(d, "в ужасе") ||
            Js.String2.includes(d, "испуган") ||
            Js.String2.includes(d, "паник") {
    "[frightened]"
  } else if Js.String2.includes(d, "потряс") {
    "[shocked]"
    ...
    "[bargaining]"
            ...
            Js.String2.includes(d, "обманутая надежд") {
    "[disappointed]"
    ...
    "[proudly]"
            ...
            Js.String2.includes(d, "обещая чудо") ||
    ...
    "[playfully]"
  } else if Js.String2.includes(d, "уверен") ||
            Js.String2.includes(d, "смелее") ||
            Js.String2.includes(d, "входя во вкус") {
    "[confidently]"
  } else if Js.String2.includes(d, "торжеств") ||
            Js.String2.includes(d, "ведущ") ||
            Js.String2.includes(d, "собирая внимание") ||
    ...
    "[slowly]"
            ...
            Js.String2.includes(d, "ясно формулируя") {
    "[seriously]"
...
let narrationFrom = (id: string, scene: string, markdown: string): (string, string) => {
  let plain =
    ...
    ->trim
  let fixed = switch id {
  | "E2V4-001" => "«Фрося и Вася». Серия вторая: «Самый крутой номер»."
  | _ => plain
  }
  let tag = if id == "E2V4-001" {
    "[warmly]"
  } else {
    switch scene {
    | "scene_04" | "scene_05" => "[playful storytelling]"
    | "scene_06" => "[building tension]"
    | "scene_07" | "scene_08" | "scene_09" => "[intense storytelling]"
    | "scene_10" => "[uneasy storytelling]"
    | "scene_11" => "[mysterious storytelling]"
    | _ => "[warm storytelling]"
    }
  }
  (tag, fixed)
...
let parseScreenplay = (): array<segment> => {
  ...
  ->Belt.Array.forEach(raw => {
    ...
    let isNumbered = Js.String2.startsWith(raw, "**E2V4-")
    if (isHeading || isNumbered) && includedBlock(currentId.contents) {
      ...
      if Js.String2.includes(raw, ":**") {
        ...
        let text = parsedText
        let kind = speaker == "CHORUS_SIBLINGS"
          ? "chorus"
          : isSpellingText(parsedText)
            ? "spelling"
            : "dialogue"
...
let directed = (s: segment): string => {
  let spoken = pronunciationForAudio(s.text)
  /* Performance tags made the model run separately spelled letters together in
     the rejected read. A spelling segment is always sent as bare text. */
  s.kind == "spelling" || s.tag == "" ? spoken : s.tag ++ " " ++ spoken
}
...
let chunkSegments = (segments: array<segment>): array<chunk> => {
  ...
  segments->Belt.Array.forEach(s => {
    if s.kind == "chorus" || s.kind == "spelling" {
      flush()
      chunkIndex := chunkIndex.contents + 1
      let _ = Js.Array2.push(chunks, {
        id: s.scene ++ "_" ++ s.kind ++ "_" ++ Belt.Int.toString(chunkIndex.contents),
        scene: s.scene,
        kind: s.kind,
        segments: [s],
...
let validate = (segments: array<segment>, chunks: array<chunk>): unit => {
  if Belt.Array.length(segments) != 222 {
    raise(TableRead(
      "expected 222 spoken spec blocks, got " ++ Belt.Int.toString(Belt.Array.length(segments)),
    ))
  }
  let dialogue = segments->Belt.Array.keep(s =>
    s.kind == "dialogue" || s.kind == "chorus" || s.kind == "spelling"
  )
  let narration = segments->Belt.Array.keep(s => s.kind == "narration")
  if Belt.Array.length(dialogue) != 117 || Belt.Array.length(narration) != 105 {
    raise(TableRead(
      "expected 117 dialogue and 105 narration blocks, got " ++
  ...
  if Belt.Array.length(choruses) != 0 {
    raise(TableRead("this screenplay should not contain a sibling chorus block"))
  }
  ...
  let untagged = segments->Belt.Array.keep(s => s.kind != "spelling" && s.tag == "")
  if Belt.Array.length(untagged) > 0 {
    raise(TableRead("every acted dialogue and narration segment must carry an expression tag"))
  }
  let taggedSpelling = segments->Belt.Array.keep(s =>
    s.kind == "spelling" && Js.String2.startsWith(directed(s), "[")
  )
  if Belt.Array.length(taggedSpelling) > 0 {
    raise(TableRead("spelling beats must stay bare so separately named letters do not run together"))
  }
  let dotLines = segments->Belt.Array.keep(s =>
    s.speaker == "FROSYA" && Js.String2.includes(s.text, "И точка!")
  )
  if Belt.Array.length(dotLines) != 8 {
    raise(TableRead(
      "expected eight Frosya materialization triggers with И точка!, got " ++
      Belt.Int.toString(Belt.Array.length(dotLines)),
    ))
  }
  let physicalDots = narration->Belt.Array.keep(s =>
    Js.String2.includes(s.text, "ставит") && Js.String2.includes(s.text, "точку")
  )
  if Belt.Array.length(physicalDots) != 8 {
    raise(TableRead(
      "expected eight narrated physical period placements, got " ++
      Belt.Int.toString(Belt.Array.length(physicalDots)),
    ))
  }
  if voiceFor("NARRATOR") != "deqzqEZ3ngCdcOl0jF1F" {
    raise(TableRead("Russian narrator must remain the author-approved Anna Zub voice"))
  }
  let notes = segments->Belt.Array.keep(s => idNumber(s.blockId) >= 220)
  if Belt.Array.length(notes) > 0 {
    raise(TableRead("production notes must not be spoken"))
  }
  let missingMagic = [
    "ВОСК", "ВАТА", "ЛАВКА", "КАСКА", "КОЛОКОЛ", "ВОЛ", "СОВА", "ВОЛК",
    "ОБВАЛ", "МОСТ", "СТВОЛ",
  ]
