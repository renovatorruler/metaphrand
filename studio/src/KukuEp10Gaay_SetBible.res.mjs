...
function plateNotes(s) {
  switch (s) {
    case "Courtyard" :
        return [];
    case "Lane" :
        return ["the light is the last golden evening — a low warm sun, long soft shadows, a dusk-gold sky"];
    case "FlatStone" :
        return [
...
function platePrompt(s, vantageOpt, param) {
  ...
  return PromptGate.pass("platePrompt", [
                ...
                "HARD RULES:\n" + Kuku_PromptSpec.bullets([
                      ...
                      ].concat(Kuku_PromptSpec.worldFacts.concat(plateNotes(s))))
              ].join("\n"));
}
...
if (exit === 1) {
  console.log("usage: node src/KukuEp10Gaay_SetBible.res.mjs <blueprint|plate|survey|prompt> <lane|courtyard|flat_stone|tower|doorway|grass_verge>  (got: " + mode + ")");
}
