...
const CUE_MEDIA = {
  ...
  S014: { video: C + "SCENE1_s0a_furia_ring.mp4" },   // «मैं उड़ रही हूँ!» — the circuit (arch-era clip retired)
  A04: { video: C + "A04_drill_full.mp4" },           // the drill: circuit + bell touch, 10s
...
execFileSync("ffmpeg", ["-v", "error", "-y", "-i", B + "picture.mp4", "-i", B + "dialogue.m4a",
  ...
  "-movflags", "+faststart", "-shortest", P + "EP10_V8_REVIEW_NUMBERED.mp4"], { timeout: 1800000 });

fs.writeFileSync(P + "EP10_V8_SHOTLIST.md",
  "# EP10 V8 — full cut shot list (S### dialogue, A## action/sound)\n\n| # | at | who | scene | frame |\n|---|----|-----|-------|-------|\n" + rows.join("\n") + "\n");
console.log(`EP10_V8_REVIEW_NUMBERED.mp4 — ${beats.length} beats, ${t.toFixed(1)}s`);
