...
EDL   = json.load(open('ep3_edl_lip.json'))
DURS  = json.load(open('ep3_durs.json'))
TAKED = {int(k): v for k, v in DURS['takes'].items()}
SFXD  = DURS['sfx']
BUILD, OUT = 'build_lip', 'out_lip'
...
run(['ffmpeg','-y','-f','concat','-safe','0','-i',f'{OUT}/ep_cat.txt','-c','copy',f'{OUT}/KUKU_EP3_V2_LIPTEST.mp4'])
d = probe(f'{OUT}/KUKU_EP3_V2_LIPTEST.mp4')
print('\n'.join(report))
for s in skipped: print('SKIPPED', s)
print(f"EPISODE: {d:.1f}s -> {OUT}/KUKU_EP3_V1.mp4")
PY
