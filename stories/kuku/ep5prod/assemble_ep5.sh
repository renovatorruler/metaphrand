...
def place_takes(seg):
    t = 0.0; placed = []; nspeech = 0
    for tk in seg.get('takes', []):
        speech = 'i' in tk
        at = tk['at'] if 'at' in tk else (LEAD if nspeech == 0 else t + GAP)
        d = ev_dur(tk)
        placed.append({'tk': tk, 'at': at, 'end': at + d, 'duck': speech or tk.get('duck', True)})
        # only speech advances the clock — effects ride underneath
        if speech:
            t = max(t, at + d); nspeech += 1
    speech = [p for p in placed if 'i' in p['tk']]
    need = (max(p['end'] for p in speech) + TAIL) if speech else 0.0
    return placed, need
...
def render_seg(scene, si, seg, dur):
    ...
    # CACHE KEY: the segment spec + its duration, not just the slot index. Keying on
    # scene+index alone silently reused an old picture when only 'src' changed — a
    # whole rewire of 93 segments once "succeeded" without changing a single frame.
    stamp = f"{BUILD}/{scene}_{si:02d}.key"
    key = json.dumps({'seg': seg, 'dur': dur}, sort_keys=True, ensure_ascii=False)
    if not (os.path.exists(stamp) and open(stamp).read() == key):
        for stale in (out, base):
    ...
    with open(stamp, 'w') as f: f.write(key)
    return out
...
for sc in EDL['scenes']:
    ...
    AMB = {'stream_loud','wind_gust','evening_crickets','hands_in_water','rope_creak','kite_flutter','run_planks'}
    speech_win = [(e['abs'], e['end']) for e in events if e['path'].startswith('takes/')]
    if events:
        args = ['ffmpeg','-y']
        for e in events: args += ['-i', e['path']]
        chains, mix = [], []
        for k, e in enumerate(events):
            nm = e['path'].split('/')[-1].rsplit('.',1)[0]
            duckf = ''
            if nm in AMB and speech_win:
                w = '+'.join(f"between(t,{max(0,a-e['abs']-0.15):.2f},{max(0,b-e['abs']+0.25):.2f})"
                             for a,b in speech_win if b > e['abs'] and a < e['end'])
                if w: duckf = f",volume='if({w},0.30,1.0)':eval=frame"
            chains.append(f"[{k}:a]aresample=44100{duckf},adelay={int(e['abs']*1000)}:all=1[d{k}]")
            mix.append(f"[d{k}]")
...
run(['ffmpeg','-y','-f','concat','-safe','0','-i',f'{OUT}/ep_cat.txt','-c','copy',f'{OUT}/KUKU_EP5_V1.mp4'])
d = probe(f'{OUT}/KUKU_EP5_V1.mp4')
print('\n'.join(report))
for s in skipped: print('SKIPPED', s)
print(f"EPISODE: {d:.1f}s -> {OUT}/KUKU_EP5_V1.mp4")
PY
