...
def emit(pts,n,hot=1.0):
    ...
    for i in idx:
        # one spark in six is a long-faller: it carries down out of the letter
        # and is what makes the light feel like it is burning rather than lit
        longlife = rng.random()<0.17
        life = rng.uniform(1.4,2.6) if longlife else rng.uniform(0.45,1.25)
        SPARKS.append([pts[i,0],pts[i,1],
                       rng.normal(0,21),rng.normal(-12,17),
                       life,0.0,rng.uniform(0.6,1.0)*hot,
                       rng.uniform(0.55,1.0)])   # colour temperature of this spark
def step_sparks(buf,dt):
    ...
    for s in SPARKS:
        ...
        s[3]+=34*dt          # gentler gravity so they drift down rather than drop
        s[2]*=0.992; s[3]*=0.997
        a=1.0-s[5]/s[4]
        flick=0.50+0.50*math.sin(s[5]*44+s[0]*0.7)
        splat(buf,np.array([[s[0],s[1]]]),s[6]*a*a*flick*2.9)
        keep.append(s)
    SPARKS[:]=keep
...
def tonemap(buf):
    ...
    halo   = gaussian_filter(lum,70.0)      # the glow hanging in the air
    bounce = gaussian_filter(lum,230.0)     # reaches across the frame to her
    return np.clip(warm+white,0,1), halo, bounce
...
for ri,(ch,dur) in enumerate(REPS):
    for i in range(int(round(dur*FPS))):
        ...
        light, halo, bounce = tonemap(buf)
        # HALO: glow hanging in the air around the letters, added everywhere
        amb=np.stack([halo*0.105, halo*0.064, halo*0.026],-1)
        # BOUNCE: light landing on things. Multiplying by the plate's own
        # luminance means it lands on her face, her arms and the paper — on
        # surfaces that can catch light — and not on empty darkness, which is
        # what the halo is for. It falls off with distance on its own.
        plum=plate.mean(-1)[...,None]
        bnc=np.stack([bounce*1.00,bounce*0.60,bounce*0.24],-1)*plum*0.85
        out=1-(1-plate)*(1-light)
        out=np.clip(out+amb+bnc,0,1)
