"""AMAL Ep1 v2 — table-read audio from EP1_PAGES_v2_HI.md. सूत्रधार (N) reads the Hindi action; the
cast performs the Hindi dialogue. Rendered PER SEGMENT (one TTS call each) so we get exact per-segment
timing for the English subtitle track. Ambient bed under it. Digest-cached. Writes ep1_v2_timing.json.
...
A = f"{D}/ep1_v2_audio"; os.makedirs(A, exist_ok=True)
...
for raw in open(f"{D}/EP1_PAGES_v2_HI.md", encoding="utf-8").read().splitlines():
    ln = raw.strip()
...
ff(["-i", f"{A}/voices.mp3", "-i", f"{A}/bed.mp3", "-filter_complex",
    ...
    "-map", "[o]", "-c:a", "libmp3lame", "-b:a", "192k", f"{D}/ep1_v2_hindi.mp3"])
...
json.dump({"gap": GAP, "voices_dur": VD,
          ...
          open(f"{D}/ep1_v2_timing.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
print(f"DONE -> ep1_v2_hindi.mp3 ({bk.duration(f'{D}/ep1_v2_hindi.mp3')/60:.1f} min) + ep1_v2_timing.json", flush=True)
